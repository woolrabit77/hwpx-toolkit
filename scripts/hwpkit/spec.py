...
from .model import Document, Note, Paragraph, Run, Table, TableCell
...
def parse_document_spec(raw: dict[str, Any]) -> tuple[Document, IdRegistry]:
    ...
    for index, block in enumerate(blocks):
        ...
        if block_type in {"paragraph", "heading"}:
            ...
            parsed_blocks.append(_parse_table(block))
...
def _parse_table(block: dict[str, Any]) -> Table:
    rows = block.get("rows", [])
